...
public class QueryActivity extends AppCompatActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		try {
			JSONArray arr1 = Caladrius.fitbitInterface.getFitbitData(statsList.get(graphStats),
				timeRangeTypeGraphs,
				startTime,
				endTime,
				graphTimeRange);
			...
			if (numberOfGraph == GraphContract.GraphEntry.GRAPH_NUMBER_TWO) {
				...
				arr2 = Caladrius.fitbitInterface.getFitbitData(statsList.get(graph2Stats),
					timeRangeTypeGraphs,
					startTime,
					endTime,
					graphTimeRange);
				for (int x = 0; x < arr2.length(); x++) {
					String dt = arr2.getJSONObject(x)
					                .getString("dateTime");
					int vl = arr1.getJSONObject(x)
					             .getInt("value");
					int v2 = arr2.getJSONObject(x)
					             .getInt("value");
				...
				for (int x = 0; x < arr1.length(); x++) {
					String dt = arr1.getJSONObject(x)
					                .getString("dateTime");
					int vl = arr1.getJSONObject(x)
					             .getInt("value");
		...
		TableRow.LayoutParams wrapWrapTableRowParams =
			new TableRow.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
				LinearLayout.LayoutParams.WRAP_CONTENT);
		int[] fixedColumnWidths = new int[]{34, 30, 30};
		int[] scrollableColumnWidths = new int[]{34, 30, 30};
		...
		for (int i = 0; i < points.size(); i++) {
			TextView fixedView = makeTableRowWithText(points.get(i)
			                                                .getDate(),
				scrollableColumnWidths[0],
				fixedRowHeight);
			...
			row.addView(makeTableRowWithText(points.get(i)
			                                       .getValue(), scrollableColumnWidths[1], fixedRowHeight));
			if (!points.get(i)
			           .getValue2()
			           .equals("-1")) {
				row.addView(makeTableRowWithText(points.get(i)
