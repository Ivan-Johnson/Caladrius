...
/**
 * A class responsible for converting Conditions to meaninful text.
 * <p>
 * NOTE: initially, it might seem as though you can easily avoid the need for this class by adding a describe function
 * to the Condition interface, but that is incorrect because these descriptions can vary depending on the App's
 * language. Since all RSS-related classes need to be able to be compiled without Android in order to run on Caladrius'
 * server, that means that the description must be generated by a separate class.
 */
