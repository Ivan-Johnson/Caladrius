...
public class Fitbit
{
	...
	public static JSONArray getFitbitData(FitbitAccount acc,
	                                      String stat,
	                                      int timeType,
	                                      String start,
	                                      String end,
	                                      int timeRange) throws JSONException, InterruptedException, ExecutionException, IOException
	{
		...
		case TIME_RANGE_TYPE_SINGLE:
			url = String.format("https://api.fitbit.com/1/user/%s/activities/%s/date/%s/1d.json",
				acc.privateToken.getUserId(),
				stat,
				getReformattedData(start));
			break;
		case TIME_RANGE_TYPE_SEVERAL:
			url = String.format("https://api.fitbit.com/1/user/%s/activities/%s/date/%s/%s.json",
				acc.privateToken.getUserId(),
				stat,
				getReformattedData(start),
				getReformattedData(end));
			break;
